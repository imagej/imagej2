...
public class ColorProcessor extends ImageProcessor {
	...
	public Image createImage() {
		if (ijx.IJ.isJava16()) return createBufferedImage();
		if (source==null) {
	...
	*/
	...
	/** Returns an IjxImageStack with three 8-bit slices,
	    representing hue, saturation and brightness */
	public IjxImageStack getHSBStack() {
		int width = getWidth();
 	...
 	final ImageProcessor filterRGB(int type, double arg, double arg2) {
		...
		switch (type) {
			...
			case RGB_RESIZE:
				...
				return ip2;
			case RGB_TRANSLATE:
				ijx.IJ.showStatus("Translating red");
				r.translate(arg, arg2); showProgress(0.40);
				ijx.IJ.showStatus("Translating green");
				g.translate(arg, arg2); showProgress(0.65);
				ijx.IJ.showStatus("Translating blue");
