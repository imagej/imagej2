			...
			var P = function(R) {
				...
				if (R.getBoundingClientRect) {
					...
					return [
							S.left
									+ B.Dom
											.getDocumentScrollLeft(R.ownerDocument),
	...
	(function() {
		...
		if (D.isIE) {
			...
			if (("undefined" !== typeof YAHOO_config) && YAHOO_config.injecting) {
				...
				E
						.write("<script id=\"_yui_eu_dr\" defer=\"true\" src=\"//:\"></script>");
...
YAHOO.util.Connect = {
							...
							function(q) {
								try {
									...
									if (S.type.toLowerCase() == "submit") {
										YAHOO.util.Connect._submitElementValue = encodeURIComponent(S.name)
												+ "="
	...
	asyncRequest : function(w, q, m, S) {
		var R = (this._isFileUpload) ? this.getConnectionObject(true) : this
				.getConnectionObject();
		if (!R) {
			return null;
		} else {
	...
	initCustomEvents : function(S, R) {
		for ( var q in R.customevents) {
			if (this._customEvents[q][0]) {
				S[this._customEvents[q][0]] = new YAHOO.util.CustomEvent(
						this._customEvents[q][1], (R.scope) ? R.scope : null);
	...
	handleTransactionResponse : function(w, V, S) {
		...
		if (R >= 200 && R < 300 || R === 1223) {
			q = this.createResponseObject(w, (V && V.argument) ? V.argument
					: undefined);
			if (V) {
				if (V.success) {
					if (!V.scope) {
			...
			case 12152:
			case 13030:
				q = this.createExceptionObject(w.tId,
						(V && V.argument) ? V.argument : undefined, (S ? S
								: false));
				if (V) {
					if (V.failure) {
						if (!V.scope) {
	...
	createResponseObject : function(S, d) {
		...
		m.statusText = (S.conn.status == 1223) ? "No Content"
				: S.conn.statusText;
		...
		for ( var m = 0; m < f.elements.length; m++) {
			...
			if (!p && T) {
				...
				case "select-multiple":
					for ( var R = 0; R < S.options.length; R++) {
						if (S.options[R].selected) {
							if (window.ActiveXObject) {
								this._sFormData += encodeURIComponent(T)
										+ "="
										+ encodeURIComponent(S.options[R].attributes["value"].specified ? S.options[R].value
												: S.options[R].text) + "&";
							} else {
								this._sFormData += encodeURIComponent(T)
										+ "="
										+ encodeURIComponent(S.options[R]
												.hasAttribute("value") ? S.options[R].value
												: S.options[R].text) + "&";
							}
						}
					}
	...
	uploadFile : function(m, p, w, R) {
		...
		var V = function() {
			...
			try {
				v.responseText = f.contentWindow.document.body ? f.contentWindow.document.body.innerHTML
						: f.contentWindow.document.documentElement.textContent;
				v.responseXML = f.contentWindow.document.XMLDocument ? f.contentWindow.document.XMLDocument
...
YAHOO.util.Anim.prototype = {
	...
	setRuntimeAttribute : function(B) {
		...
		this.runtimeAttributes[B].unit = (F(D[B].unit)) ? D[B]["unit"] : this
				.getDefaultUnit(B);
		return true;
	},
	init : function(C, H, G, A) {
		...
		this.animate = function() {
			...
			this.totalFrames = (this.useSeconds) ? Math
					.ceil(YAHOO.util.AnimMgr.fps * this.duration)
					: this.duration;
		...
		var I = function() {
			...
			for ( var K in L) {
				this.setAttribute(K, this.doMethod(K, L[K].start, L[K].end),
						L[K].unit);
			}
			F += 1;
		};
		var E = function() {
			...
			L.toString = function() {
				return ("duration: " + L.duration + ", frames: " + L.frames
						+ ", fps: " + L.fps);
			};
			B = false;
			F = 0;
...
(function() {
	...
	A.doMethod = function(E, I, F) {
		...
		if (this.patterns.color.test(E)) {
			...
			H = "rgb(" + Math.floor(H[0]) + "," + Math.floor(H[1]) + ","
					+ Math.floor(H[2]) + ")";
		} else {
	...
	A.setRuntimeAttribute = function(E) {
		...
		if (this.patterns.color.test(E)) {
			...
			if (typeof G[E]["to"] === "undefined"
					&& typeof G[E]["by"] !== "undefined") {
...
(function() {
	...
	B.getAttribute = function(F) {
		if (this.patterns.points.test(F)) {
			var G = [ E.getAttribute.call(this, "left"),
					E.getAttribute.call(this, "top") ];
		} else {
...
(function() {
	...
	A.doMethod = function(D, G, E) {
		...
		if (D == "scroll") {
			F = [
					this.method(this.currentFrame, G[0], E[0] - G[0],
							this.totalFrames),
					this.method(this.currentFrame, G[1], E[1] - G[1],
							this.totalFrames) ];
		} else {
		...
		return {
			...
			handleMouseMove : function(E) {
				...
				if (B) {
					...
					if (!this.dragThreshMet) {
						var D = Math.abs(this.startX
								- YAHOO.util.Event.getPageX(E));
						var C = Math.abs(this.startY
								- YAHOO.util.Event.getPageY(E));
						if (D > this.clickPixelThresh
								|| C > this.clickPixelThresh) {
			...
			fireEvents : function(Q, H) {
				...
				var J = YAHOO.util.Event.getPageX(Q), I = YAHOO.util.Event
						.getPageY(Q), K = new YAHOO.util.Point(J, I), F = S
						.getTargetCoord(K.x, K.y), C = S.getDragEl(), P = new YAHOO.util.Region(
						F.y, F.x + C.offsetWidth, F.y + C.offsetHeight, F.x), E = [], G = [], B = [], R = [], O = [];
				...
				this.interactionInfo = {
					...
					point : K,
					draggedRegion : P,
					sourceRegion : this.locationCache[S.id],
					validDrop : H
				};
				if (H && !R.length) {
				...
				if (C == 1) {
					...
					for ( var E = 0; E < C; ++E) {
						...
						if (this.mode == this.INTERSECT && B.cursorIsOver) {
							...
							if (!F
									|| !F.overlap
									|| (B.overlap && F.overlap.getArea() < B.overlap
											.getArea())) {
								F = B;
							}
						}
			...
			isOverTarget : function(J, B, D, E) {
				...
				if (!E) {
					...
					var C = I.getDragEl();
					E = new YAHOO.util.Region(G.y, G.x + C.offsetWidth, G.y
							+ C.offsetHeight, G.x);
				}
				var H = E.intersect(F);
				if (H) {
...
YAHOO.extend(YAHOO.util.DD, YAHOO.util.DragDrop, {
	...
	alignElWithMouse : function(B, F, E) {
		...
		if (!this.deltaSetXY) {
			...
			YAHOO.util.Dom.setStyle(B, "left", (D.x + this.deltaSetXY[0])
					+ "px");
			YAHOO.util.Dom
					.setStyle(B, "top", (D.y + this.deltaSetXY[1]) + "px");
		}
...
(function() {
	...
	YAHOO.util.Element.prototype = {
		...
		setStyle : function(I, H) {
			...
			if (!G) {
				return this._queue[this._queue.length] = [ "setStyle",
						arguments ];
			}
