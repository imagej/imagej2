 ...
 * This finds the underscored .jar/.zip and .class files in plugins directory
 * (plus one level down) and parses the plugin.config file from the jars
 *
 * ** There is some remaining detritis from ij.Menu that needs to be cleaned out.
 *
 * - GBH, Dec 2010
 */
public class UserPluginFinder implements PlugIn, PluginFinder {
    //
...
//                + "" + "");
//        System.out.println("");
        // @TODO Change constructor with arg.
...
//                );
    ...
    String strLeft(int w, String s) {
        if(s==null) s = "";
        // return new imagej.util.StringAlign(w, StringAlign.JUST_LEFT).format(s);
        return "";
    }

    public static void main(String[] args) {
        ArrayList<PluginEntry> plugins = new ArrayList<PluginEntry>();
        UserPluginFinder pLoader = new UserPluginFinder();
