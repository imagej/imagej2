...
public class MenuBuilderIJ {

    public JMenuBar buildMenuBar(final List<PluginEntry> entries) {
        return buildMenus(new JMenuBar(), entries);
    }

    public JMenuBar buildMenus(final JMenuBar menubar, final List<PluginEntry> entries) {
        for (final PluginEntry entry : entries) {
            ...
            IOProvider.getDefault().getIO("PluginFinder", false).getOut().println("Installed: " + entry);
        }
        return menubar;
    }
