...
public class ControlPanel implements PlugIn {
    ...
    private synchronized DefaultMutableTreeNode doRootFromMenus() {
        ...
        if (m instanceof MenusAWT) {
            MenuBar menuBar = (MenuBar) m.getMenuBar();
    ...
    void saveProperties() {
        if (IJ.debugMode) {
            IJ.log("CP.saveProperties: " + propertiesChanged);
        }
...
class TreePanel implements
    ...
    public void buildTreePanel() {
        ...
        if (defaultLocation != null) {
            if (IJ.debugMode) {
                IJ.log("CP.buildTreePanel: " + defaultLocation);
            }
