...
function reportAbuse() {
	...
	location.href = "mail" + "to:" + "pipes-pipeabuse" + "@"
			+ "yahoo-inc.com?subject=Reporting abuse for Pipe: " + _5
			+ "&body=" + _5 + "%0A Please state why this Pipe is abusive:";
}
function fbs_click() {
	u = location.href;
	t = document.title;
		...
		success : function(o) {
			if (o.responseText) {
				...
				if (typeof (_19.ok) == "undefined") {
					_12("Oops. System error 1999: badly formed response");
					YAHOO.log(_19);
					return;
				}
				if (!_19.ok) {
...
YAHOO.pipes.site = function() {
	...
	function loadComponent(_31, _32, _33) {
		...
		YAHOO.util.Connect.asyncRequest("GET", _31, {
			"success" : onSuccess,
			"failure" : onFailure,
			"argument" : [ _32, _33 ]
		});
...
YAHOO.pipes.site.subscribe = function(_46) {
	...
	YAHOO.util.Event.addListener(_46, "mouseover", this.containerMouseOver,
			this, true);
	YAHOO.util.Event.addListener(_46, "mouseout", this.containerMouseOut, this,
			true);
...
YAHOO.pipes.site.thumb.prototype.drawLine = function(x, y, x2, y2, _7c, _7d) {
	...
	ctx.bezierCurveTo(_80[1][0], _80[1][1], _80[2][0], _80[2][1], _80[3][0],
			_80[3][1]);
	ctx.stroke();
	w = 2;
	ctx.lineWidth = w;
...
YAHOO.pipes.site.InlineTextEdit.prototype.init = function() {
	...
	if (!this.node) {
		...
		if (!this.inparent) {
			...
			YAHOO.util.Dom.get(this.originalNode).parentNode
					.appendChild(this.node);
		}
		if (!this.nobuttons) {
			YAHOO.util.Event.addListener(this.ok, "click", this.inviteOk, this,
					true);
			YAHOO.util.Event.addListener(this.cancel, "click",
					this.inviteCancel, this, true);
		}
		YAHOO.util.Event.addListener(this.input, "blur", this.checkBlur, this,
				true);
		var _8d = [ "fontSize", "fontWeight", "fontFamily", "lineHeight",
				"color" ];
		for ( var i in _8d) {
			YAHOO.util.Dom.setStyle(this.input, _8d[i], YAHOO.util.Dom
					.getStyle(this.originalNode, _8d[i]));
		}
	}
	if (this.expandright) {
...
YAHOO.pipes.site.InlinePermalinkEdit = function(_9c, _9d, _9e, _9f) {
	...
	this.prefix = _9d;
	this.suffix = this.originalUrl.substring(_9d.length,
			this.originalUrl.length);
	_9c.innerHTML = _9d + "<span>" + this.suffix + "</span>";
	this.edit = new YAHOO.pipes.site.InlineTextEdit(_9c
			.getElementsByTagName("span")[0], false, _9e, false, true);
...
YAHOO.pipes.site.InlineTagEdit.prototype.init = function() {
	if (!this.node) {
		...
		var _aa = "<input size='" + (this.emptytext.length)
				+ "'class='taginput' type='text' value='" + this.emptytext
				+ "' /><img class='tagadd' src='space.gif'  />";
		this.node = document.createElement("div");
		...
		YAHOO.util.Event
				.addListener(this.add, "click", this.addTag, this, true);
		YAHOO.util.Event.addListener(this.input, "focus", this.inputfocus,
				this, true);
		YAHOO.util.Event.addListener(this.input, "blur", this.inputblur, this,
				true);
		YAHOO.util.Event.addListener(this.input, "keydown", this.keydown, this,
				true);
	}
};
