...
public class FileOpener {
	...
	public void revertToSaved(IjxImagePlus imp) {
		...
		if (fi.fileFormat==fi.BMP) {
			// restore BMP
			IjxImagePlus imp2 = (IjxImagePlus)IJ.runPlugIn("ijx.plugin.readwrite.BMP_Reader", path);
			if (imp2!=null)
				imp.setProcessor(null, imp2.getProcessor());
	    	return;
		}
		...
		if (fi.fileFormat==fi.PGM) {
			// restore PGM
			IjxImagePlus imp2 = (IjxImagePlus)IJ.runPlugIn("ijx.plugin.readwrite.PGM_Reader", path);
			if (imp2!=null)
				imp.setProcessor(null, imp2.getProcessor());
	    	return;
		}
		...
		if (fi.fileFormat==fi.FITS) {
			// restore FITS
			IjxImagePlus imp2 = (IjxImagePlus)IJ.runPlugIn("ijx.plugin.readwrite.FITS_Reader", path);
			if (imp2!=null)
				imp.setProcessor(null, imp2.getProcessor());
			return;
		}
