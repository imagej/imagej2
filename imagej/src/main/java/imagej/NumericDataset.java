...
public class NumericDataset
{
	...
	private class PositionMatchesBooleanFunction implements BooleanFunction
	{
	...
	private interface NumericSet
	{
		boolean includes(int[] position);
	}
	
	private class AndNumericSet implements NumericSet
	{
		NumericSet a, b;
		
		public AndNumericSet(NumericSet a, NumericSet b)
		{
			this.a = a;
			this.b = b;
		}
		...
		{
			return a.includes(position) && b.includes(position);
		}
	}
	
	private class OrNumericSet implements NumericSet
	{
		NumericSet a, b;
		
		public OrNumericSet(NumericSet a, NumericSet b)
		{
			this.a = a;
			this.b = b;
		}
	...
	private class NotNumericSet implements NumericSet
	{
		NumericSet a;
		
		public NotNumericSet(NumericSet a)
		{
			this.a = a;
		}
		...
		public boolean includes(int[] position)
		{
			return ! (a.includes(position));
		}
	}
	
	private class IncludePlane implements NumericSet
	{
		private int axisNumber;
		private int planeNumber;
		
		public IncludePlane(int axisNumber, int planeNumber)
		{
			this.axisNumber = axisNumber;
			this.planeNumber = planeNumber;
		}

		// NOTE: assumes position is within bounds of set
		public boolean includes(int[] position)
		{
			if (position[axisNumber] == planeNumber)
				return true;
	...
	// this could be implemented as a bunch of ANDs
	
	private class SubRegion implements NumericSet
	{
		int[] axes, planes;
		
		public SubRegion(int[] axes, int[] planes)
		{
			if (axes.length != planes.length)
				throw new IllegalArgumentException("axes size doesn't match planes size");
	...
	// TODO : define more Operations that use a selector (Binary/NAry & Assign/Transform)
	//  Also think how to generalize these Operations so that we have less class explosion
	
	private class ConditionalPositionalSingleCursorOperation<T extends RealType<T>> extends PositionalSingleCursorRoiOperation<T>
	{
		private SelectionFunction selector;
		private UnaryComputation computer;
		
		public ConditionalPositionalSingleCursorOperation(Image<T> image, int[] origin, int[] span,
															SelectionFunction selector, UnaryFunction function)
		{
		...
		{
			if (selector.include(position, sample.getRealDouble()))
