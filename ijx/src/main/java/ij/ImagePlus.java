 ...
 * Fields arranged into functional groupings.
 **/
public class ImagePlus implements IjxImagePlus {
    ...
    private int ID;
    private static Component comp;  // @todo Unnecessary??
    ...
    public void draw() {
        if (win != null) {
            if (win.getCanvas() != null) {
                ...
                System.err.println("win.getCanvas()==null in ImagePlus.draw()");
            }
        }
    }
...
//				if (IJ.getListeners().size()>0) notifyListeners(UPDATED);
    ...
    public void copy(boolean cut) {
        ...
        if (roi2 != null) {
            IJ.getInstance().getClipboard().setRoi(roi2);
        }
        if (cut) {
            ip.snapshot();
