 ...
 */
// from package net.java.dev.imagine;
...
public final class ApplicationContext implements ContextGlobalProvider {
  ...
  public ApplicationContext() {
    ...
    // a module providing a lookup we are proxying is installed/uninstalled
    LookupListener ll = new LookupListener() {

      public void resultChanged(LookupEvent evt) {
        updateLookups(((Lookup.Result) evt.getSource()).allInstances());
      }
    };
  ...
  private void updateLookups(Collection allContributors) {
    SelectionContextContributor[] p =
            (SelectionContextContributor[]) allContributors.toArray(
            new SelectionContextContributor[allContributors.size()]);
