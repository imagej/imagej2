...
then
	...
	unzip -o $ZIP &&
	contents="$(unzip -l $ZIP | sed -n 's/^.*\(ImageJ2.app\/.*[^\/]\)$/\1/p')" &&
	possibly_obsoletes="$(printf "%s\n%s\n%s" \
			"$contents" "$contents" "$(find ImageJ2.app -type f)" |
		grep -ve /.checksums$ -e /db.xml.gz -e ^ImageJ2.app/samples/ \
...
for jar in ImageJ2.app/jars/*.jar
do
