...
public class OpenAction<T extends RealType<T>> extends AbstractAction {
	...
	public void NOTactionPerformed(ActionEvent e) {
 ...
 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		dataset = GradientImage.createDataset(512, 512);
		try {
			iPanel = new NavigableImageFrame();
			iPanel.setDataset(dataset);
			//iPanel.setInitSize(new Dimension(600, 600));
			scrlPane = new JScrollPane(iPanel);
			tc.removeAll();
			tc.setDisplayName(dataset.getMetaData().getLabel());
			tc.setLayout(new BorderLayout());
			tc.add(scrlPane, BorderLayout.CENTER);
			//tc.repaint();
			tc.validate();
